  ...
  methods: {
    initVal () {
      if ((this.valueSeparator || this.valueSeparator !== '') && this.currentValue && this.currentValue !== '') {
        const [year = '', month = '', day = ''] = this.currentValue.split(this.valueSeparator)
    ...
    sure () {
      ...
      this.dispatch('DpzVcPicker', 'ok', this.date)
    },
    change (target, current) {
      this.$nextTick(() => {
        switch (target) {
        case 'year':{
          const lastYear = this.yearList.length - 1
        ...
        case 'month':{
          const lastMonth = this.monthList.length - 1
        ...
        case 'day':{
          const lastDay = this.dayList.length - 1
