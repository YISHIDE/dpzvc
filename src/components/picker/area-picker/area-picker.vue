  ...
  methods: {
    initVal () {
      if ((this.valueSeparator || this.valueSeparator !== '') && this.currentValue && this.currentValue !== '') {
        const [provinceName, cityName, districtName] = this.currentValue.split(this.valueSeparator)

        if (provinceName || cityName || districtName) {
          const provinces = CHINA_AREA[rootCode]
          ...
          const citys = CHINA_AREA[this.province.code]
          ...
          const districts = CHINA_AREA[this.city.code]
    ...
    _filter (target, val = {}) {
      ...
      case 'province':{
        ...
        return Object.keys(provinces).map((item, index) => {
          return { code: item, value: provinces[item], target, index }
        })
      }
      case 'city':
      case 'district': {
        const list = CHINA_AREA[val.code]
        ...
        return Object.keys(list).map((item, index) => {
          return { code: item, value: list[item], target, index }
        })
      }
      }
    },
