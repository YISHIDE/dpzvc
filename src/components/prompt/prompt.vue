  ...
  >
    ...
    <template slot="footer">
      <v-button
        ...
        @click="ok"
      >
        <span slot="button-inner">{{ okText }}</span>
      </v-button>
      <v-button
        :styles="{background:'#ffffff',color:'#ccc'}"
        @click="cancle"
      >
        <span slot="button-inner">{{ cancleText }}</span>
      </v-button>
    </template>
  </Modal>
</template>
...
export default {
  ...
  methods: {
    ok () {
      let valid = true
      if (this.validator) {
        const type = Object.prototype.toString.call(this.validator)
        switch (type) {
        case '[object RegExp]':
